...
export default function BackToTop() {
    ...
    useEffect(() => {
        const changeColor = () => {
            if (window.pageYOffset > document.body.offsetHeight - 1480) {
                setIsFooter(true);
            } else {
                setIsFooter(false);
            }
    ...
    return (
        <div onClick={backToTop}>
            <BsArrowUpSquareFill
                className={clsx(
                    isVisible
                        ? "transition-all ease-in-out duration-500 float-right cursor-pointer fixed right-[24px] bottom-[24px] z-[300] w-[44px] h-[44px] md:right-[54px] md:bottom-[54px] lg:w-[58px] lg:h-[58px]"
                        : "fixed right-[33px] -bottom-14 transition-all ease-in-out duration-500",
                    isFooter
                        ? "text-[#393d3d] hover:text-[#505757] active:text-[#000000]"
                        : "text-blue-600 hover:text-blue-400 active:text-blue-700"
                )}
            />
